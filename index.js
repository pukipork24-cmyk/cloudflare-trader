  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/bitget-balance') {
      return new Response(
        JSON.stringify({ 
          success: true,
          totalUSD: 10000 + Math.random() * 5000,
          totalMYR: 45000 + Math.random() * 10000,
          availableUSD: 8000 + Math.random() * 2000,
          availableMYR: 40000 + Math.random() * 5000
        }),
        { headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } }
      );
    }
    ...
    if (url.pathname === '/api/bitget/trades') {
      const mockTrades = [
        { symbol: 'BTCUSDT', side: 'BUY', size: 0.1, price: 43250, pnl: 150, timestamp: new Date().toISOString() },
        { symbol: 'ETHUSDT', side: 'SELL', size: 2.0, price: 2280, pnl: -50, timestamp: new Date(Date.now() - 3600000).toISOString() }
      ];
      return new Response(
        JSON.stringify({ success: true, trades: mockTrades }),
        { headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } }
      );
    }
    ...
    if (url.pathname === '/api/trades') {
      const mockTrades = [
        { symbol: 'BTCUSDT', side: 'BUY', status: 'CLOSED', pnl: 150, entry_price: 42000, exit_price: 43250 },
        { symbol: 'ETHUSDT', side: 'SELL', status: 'CLOSED', pnl: -50, entry_price: 2300, exit_price: 2280 }
      ];
      return new Response(
        JSON.stringify({ success: true, trades: mockTrades }),
        { headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } }
    ...
    if (url.pathname === '/api/intelligence-logs') {
      const mockLogs = [
        { agent: 'Technical Analysis', signal: 'BUY', confidence: 75, timestamp: new Date().toISOString() },
        { agent: 'Sentiment Analysis', signal: 'HOLD', confidence: 60, timestamp: new Date(Date.now() - 300000).toISOString() }
      ];
      return new Response(
        JSON.stringify({ success: true, logs: mockLogs }),
        { headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } }
      );
    }

    // ── /api/settings/symbol ────────────────────────────────────────────────
    if (url.pathname === '/api/settings/symbol') {
      if (request.method === 'GET') {
        return new Response(
          JSON.stringify({ success: true, symbol: 'BTCUSDT' }),
          { headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } }
        );
      } else if (request.method === 'POST') {
        return new Response(
          JSON.stringify({ success: true, message: 'Symbol updated' }),
          { headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } }
        );
      }
    }
    ...
    if (url.pathname === '/api/bot/status') {
      return new Response(
        JSON.stringify({ success: true, active: true, mode: 'auto' }),
        { headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } }
      );
    }

    // ── /api/bot/toggle ────────────────────────────────────────────────
    if (url.pathname === '/api/bot/toggle') {
      return new Response(
        JSON.stringify({ success: true, active: false, message: 'Bot toggled' }),
        { headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } }
      );
    }
    ...
    if (url.pathname === '/api/login') {
      return new Response(
        JSON.stringify({ success: true, token: 'mock_token_' + Date.now() }),
        { headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } }
      );
    }
